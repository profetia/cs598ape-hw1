...
void ImageTexture::maskImage(unsigned char r, unsigned char g, unsigned char b,
                             unsigned char rm, unsigned char gm,
                             unsigned char bm, unsigned char m) {
  int x, y;
  for (y = h - 1; y >= 0; y--)
    for (x = 0; x < w; x++) {
      int total = 4 * (x + y * w);
      if (imageData[total] == r && imageData[total + 1] == g &&
          imageData[total + 2] == b) {
        imageData[total] = rm;
...
void ImageTexture::maskImageA(unsigned char r, unsigned char g, unsigned char b,
                              unsigned char m) {
  int x, y;
  for (y = h - 1; y >= 0; y--)
    for (x = 0; x < w; x++) {
...
void ImageTexture::maskImageU(unsigned char r, unsigned char g, unsigned char b,
                              unsigned char m) {
  int x, y;
  for (y = h - 1; y >= 0; y--)
    for (x = 0; x < w; x++) {
...
void ImageTexture::maskImage(unsigned char r, unsigned char g,
                             unsigned char b) {
  int x, y;
  for (y = h - 1; y >= 0; y--)
    for (x = 0; x < w; x++) {
...
void ImageTexture::maskImage(ColorTexture b, unsigned char m) {
  ...
  for (y = h - 1; y >= 0; y--)
    for (x = 0; x < w; x++) {
      int total = 4 * (x + y * w);
      if (imageData[total] == b.r && imageData[total + 1] == b.g &&
          imageData[total + 2] == b.b) {
        imageData[total + 3] = m;
      }
    }
}
...
void ImageTexture::getColor(unsigned char *toFill, double *am, double *op,
                            double *ref, unsigned int x, unsigned int y) {
  int start = 4 * (x + w * y);
...
ImageTexture::ImageTexture(unsigned char *data, unsigned int ww,
                           unsigned int hh)
    : Texture(.3, 1., 0.) {
  imageData = data;
  w = ww;
